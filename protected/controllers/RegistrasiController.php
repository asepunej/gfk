    ...
    public function actionDaftarRegistrasi() {
       ...
       if(isset($_POST['txttgl'])and ($_POST['txttgl']!='')) {
        ...
        if($_POST['action']=='statusdelete') {
            ...
            $cmd->delete('pendaftaran.registrasi', 'id_registrasi=:id_registrasi', array(':id_registrasi' => $_POST['id_registrasi']));
        } 
        ...
        $sql="SELECT 
            ...
            FROM pendaftaran.registrasi t1
            ...
            LEFT join public.pegawai t9 on t1.id_medis=t9.id_pegawai
...
//--------------------------------------------------------
    ...
    public function actionInsertRegistrasi() {
        ...
        if($_POST['action']=='simpan') {   
            if($_POST['no_rm']=='') {    
                    ...
                    $no_rm=$RecordData[no_rm];
                        ...
                        $cmd = Yii::app()->db->createCommand();
                        $cmd->insert('pendaftaran.registrasi', array(                       
                            'no_rm'=> $no_rm,
                            ...
                            'id_petugas' => Yii::app()->session['id_pegawai'],
                            'id_user' => Yii::app()->session['id_user'],
                            'tgl'  =>date("Y-m-d h:i:sa"), 
                            'kd_klinik_reg'  =>  Yii::app()->session['kd_klinik'],                          
                            ));
                        $RecordData=Yii::app()->db->createCommand("SELECT max(id_registrasi) id_registrasi FROM pendaftaran.registrasi;")->queryRow();           
        ...
        $statussimpan=true ;            
        $this->redirect(array("Registrasi/DaftarRegistrasi"));
    ...
    $dataDokter=Yii::app()->db->createCommand("SELECT t1.id_pegawai,t1.nip,t1.nama || ' - '||t2.jabatan as nama FROM public.pegawai t1 
                                                LEFT JOIN support.jabatan t2 on t1.kd_jabatan=t2.kd_jabatan
                                                where t1.kd_jabatan in ('1','2','3') ORDER BY t1.kd_jabatan ")->queryAll();

    $dataPetugas=Yii::app()->db->createCommand("SELECT t1.id_pegawai,t1.nip,t1.nama|| ' - '||t2.jabatan as nama  FROM public.pegawai t1 
                                                LEFT JOIN support.jabatan t2 on t1.kd_jabatan=t2.kd_jabatan
